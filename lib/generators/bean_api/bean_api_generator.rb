...
class BeanAPIGenerator < Rails::Generators::NamedBase
  ...
  def create_belongs_to_entity
    belongs_to_associations.each do |association|
      unless ("#{namespace_classify}::Entities::Bean::#{simple_entity_name(association.klass)}".constantize rescue nil)
  ...
  def create_bean_api
    content = <<~CONTENT
      ...
      class #{namespace_classify}::V1::#{resource_class.name.pluralize} < API
  ...
  def mount_api
    inject_into_file(
      "#{api_path}/v1.rb",
      "\n  mount ::#{namespace_classify}::V1::#{resource_class.name.pluralize}",
  ...
  def create_entity_file(klass = resource_class)
    ...
    content << <<~FILE
      ...
      module #{namespace_classify}::Entities::Bean
    ...
    create_file "#{api_path}/entities/#{klass.name.underscore}.rb", content.join
