  ...
  class WechatController < ApplicationController
    ...
    def auth_callback
      ...
      if params[:auth_code] && params[:expires_in]
        ...
        wechat_application = WechatThirdPartyPlatform::Application.find_or_initialize_by(appid: auth_info["authorizer_appid"])
        wechat_application.binded_project_application_id = project_application.id if wechat_application.new_record?
        ...
        if wechat_application.id
          ...
          @message = "授权失败，该小程序曾经绑定过其他应用，不能再绑定当前应用" and return if wechat_application.binded_project_application_id != project_application.id
          @message = "授权失败，该应用曾经绑定过其他小程序，不能再绑定当前小程序" and return if WechatThirdPartyPlatform::Application.where(binded_project_application_id: project_application.id).where.not(id: wechat_application.id).first
        end
